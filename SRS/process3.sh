...
cat  a.csv | grep "{$ZAM}" | grep -v "Внутри TI" | grep -v "Отменено" | grep -v "Удалено" | grep "На согласование" |perl -i -npe "s/№/\\\No/g" > stage1.tobeagreed.csv 
cat  a.csv | grep "{$ZAM}" | grep -v "Внутри TI" | grep -v "Отменено" | grep -v "Удалено" | grep -v "На согласование" |perl -i -npe "s/№/\\\No/g" > stage1.csv 
cat  a.csv | grep "{$ZAM}" | grep -v "Внутри TI" | grep -v "Отменено" | grep -v "Удалено" | grep "Протокол встречи от 04.04.2014" |perl -i -npe "s/№/\\\No/g" > 20140404.csv 
cat  a.csv | grep "{$ZAM}" | grep -v "Отменено" | grep "Внутри TI" |perl -i -npe "s/№/\\\No/g" > insideTI.csv 
