...
cat  a.csv | grep "{$ZAM}" | grep -v "Внутри TI" | grep -v "Отменено" | grep "На согласование" |perl -i -npe "s/№/\\\No/g" > stage1.tobeagreed.csv 
cat  a.csv | grep "{$ZAM}" | grep -v "Внутри TI" | grep -v "Отменено" | grep -v "На согласование" |perl -i -npe "s/№/\\\No/g" > stage1.csv 
cat  a.csv | grep "{$ZAM}" | grep -v "Внутри TI" | grep -v "Отменено" | grep "Протокол встречи от 04.04.2014" |perl -i -npe "s/№/\\\No/g" > 20140404.csv 
