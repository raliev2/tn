...
public class SearchPageController extends AbstractSearchPageController
{
			...
			throws CMSItemNotFoundException
	{
		...
		performAdvancedSearch(searchQuery, onlyProductIds, isCreateOrderForm, page, showMode, sortCode, model);
			...
			final String sortCode, final Model model)
	{
		...
		if (StringUtils.isNotBlank(keywords))
		{
			if (onlyProductIds || isCreateOrderForm)
			{
				...
				searchPageData = b2bProductFlexibleSearchFacade.searchForSkus(productIdsList, pageableData,
						Arrays.asList(ProductOption.URL, ProductOption.IMAGES));
			}
			else
			{
				// search using solr.
	...
	private void addMetaData(final Model model, final String metaPrefixKey, final String searchText, final String metaPostfixKey,
