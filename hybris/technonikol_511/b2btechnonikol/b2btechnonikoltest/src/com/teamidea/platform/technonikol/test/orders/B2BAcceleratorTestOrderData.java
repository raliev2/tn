 ...
 *  
 */
package com.teamidea.platform.technonikol.test.orders;
...
import com.teamidea.platform.technonikol.test.services.AccountManagerApproveScheduleService;
...
public class B2BAcceleratorTestOrderData
{
	...
	public void createSampleOrders()
	{
		...
		placeOrderWithInCustomerPermissions(B2btechnonikolTestConstants.PAY_USER_UID, "ORDER_SENT_NOTIFICATION_SENT paid", false);

		placeOrderRequiringApproval(B2btechnonikolTestConstants.HISTORICAL_USER_UID, "PENDING_APPROVAL");
		placeOrderForQuoteNegotiation(B2btechnonikolTestConstants.HISTORICAL_USER_UID, "PENDING_QUOTE");
		placeOrderAndApproveByB2BApprover(B2btechnonikolTestConstants.HISTORICAL_USER_UID, "ORDER_SENT_NOTIFICATION_SENT Approved");
		placeOrderWithInCustomerPermissions(B2btechnonikolTestConstants.HISTORICAL_USER_UID,
				"ORDER_SENT_NOTIFICATION_SENT costcenter", true);

		placeReplenishmentOrderInCancelledState(B2btechnonikolTestConstants.REPENISHMENT_USER_UID, "REPLENISHMENT_ENDED");
		placeReplenishmentOrderInActiveState(B2btechnonikolTestConstants.REPENISHMENT_USER_UID, "REPLENISHMENT_ONGOING");
		placeReplenishmentOrderScheduleForFuture(B2btechnonikolTestConstants.REPENISHMENT_USER_UID, "REPLENISHMENT_FUTURE");

		placeOrderQuoteRequestWithMerchantResponse(B2btechnonikolTestConstants.QUOTE_USER_UID, "APPROVED_QUOTE");
		placeOrderQuoteRequestWithMerchantResponse(B2btechnonikolTestConstants.QUOTE_USER_UID, "REJECTED_QUOTE");
		placeOrderExceedingCreditLimit(B2btechnonikolTestConstants.QUOTE_USER_UID, "PENDING_MERCHANT_APPROVAL");
		placeOrderExceedingCreditAndRejected(B2btechnonikolTestConstants.QUOTE_USER_UID, "REJECTED_BY_MERCHANT");
	...
	{
		createPaymentInfo(B2btechnonikolTestConstants.PAY_USER_UID, "USD", createVisaCardInfo(), createUsBillingInfo());
		createPaymentInfo(B2btechnonikolTestConstants.PAY_USER_UID, "USD", createMasterCardInfo(), createGermanyBillingInfo());
	}
	...
	protected String placeReplenishmentOrderInCancelledState(final String userID, final String purchaseOrderNumber)
	{
		...
		try
		{
			...
			code = scheduleOrder(userID, products, createUsAddressData(), purchaseOrderNumber,
					B2btechnonikolTestConstants.STANDARD_COSTCENTER, null, triggerData).getJobCode();
			if (code != null)
			{
			...
			final boolean useCostCenter)
	{
		...
		try
		{
			...
			final OrderData orderData = placeOrder(userID, products, createUsAddressData(), purchaseOrderNumber,
					useCostCenter ? B2btechnonikolTestConstants.STANDARD_COSTCENTER : null, null);
	...
	protected String placeOrderRequiringApproval(final String userID, final String purchaseOrderNumber)
	{
		...
		try
		{
			...
			final OrderData orderData = placeOrder(userID, products, createUsAddressData(), purchaseOrderNumber,
					B2btechnonikolTestConstants.STANDARD_COSTCENTER, null);
	...
	protected String placeOrderForQuoteNegotiation(final String userID, final String purchaseOrderNumber)
	{
		...
		try
		{
			...
			final OrderData orderData = placeOrder(userID, products, createUsAddressData(), purchaseOrderNumber,
					B2btechnonikolTestConstants.STANDARD_COSTCENTER, "Please approve request");
	...
	protected String placeOrderExceedingCreditAndRejected(final String userID, final String purchaseOrderNumber)
	{
		...
		try
		{
			...
			final OrderData orderData = placeOrder(userID, products, createUsAddressData(), purchaseOrderNumber,
					B2btechnonikolTestConstants.LOWLIMIT_COSTCENTER, null);
